...
public class ArcProgress extends View {
    private Paint paint;
    protected Paint progressTextPaint;
    protected Paint suffixTextPaint;
    ...
    protected void initPainters() {
        ...
        suffixTextPaint = new TextPaint();
        suffixTextPaint.setColor(textColor);
        suffixTextPaint.setTextSize(suffixTextSize);
        suffixTextPaint.setAntiAlias(true);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(text)) {
            progressTextPaint.setColor(textColor);
            progressTextPaint.setTextSize(textSize);
            float textHeight = progressTextPaint.descent() + progressTextPaint.ascent();
            float textBaseline = (getHeight() - textHeight) / 2.0f;
            canvas.drawText(text, (getWidth() - (progressTextPaint.measureText(text) + suffixTextPaint.measureText(suffixText) + suffixTextPadding)) / 2.0f, textBaseline, progressTextPaint);

            float suffixHeight = suffixTextPaint.descent() + suffixTextPaint.ascent();
            canvas.drawText(suffixText, (getWidth()  + suffixTextPaint.measureText(text)) / 2.0f + suffixTextPadding, textBaseline + textHeight - suffixHeight, suffixTextPaint);
        }
        ...
        if (!TextUtils.isEmpty(getBottomText())) {
            progressTextPaint.setTextSize(bottomTextSize);
            float bottomTextBaseline = getHeight() - arcBottomHeight - (progressTextPaint.descent() + progressTextPaint.ascent()) / 2;
            canvas.drawText(getBottomText(), (getWidth() - progressTextPaint.measureText(getBottomText())) / 2.0f, bottomTextBaseline, progressTextPaint);
        }
    }
